...
# Прокси с авторизацией (формат: server:username:password)
PROXIES = [
...
async def parse_cars():
    ...
    async with async_playwright() as p:
        while len(all_cars) < 10:
            proxy = PROXIES[proxy_index % len(PROXIES)]
            print(f"\n[Запрос {len(all_cars) + 1}] Прокси: {proxy['server']}")
            ...
            try:
                ...
                # Проверяем IP через proxy
                try:
                    await page.goto("https://api.ipify.org?format=json", timeout=15000)
                    ip_data = await page.content()
                    print(f"  IP: OK")
                except:
                    print(f"  IP: не проверен")
                ...
                print(f"  Найдено авто: {len(car_data)}, Всего: {len(all_cars)}/10")
